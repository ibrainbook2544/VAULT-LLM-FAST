...
> 我的另一个仓库 [llm-wiki-skills](https://github.com/ibrainbook2544/llm-wiki-skills)，比较严格地遵循了 [Andrej Karpathy 的 LLM Wiki 范式](https://karpathy.github.io/)，实现了一套 `LLM-Wiki`。
...
- 在 **PKM** 方面，融合了 [Andrej Karpathy 的 LLM Wiki 范式](https://karpathy.github.io/) 与本人研究的 **FAST 分子笔记本**方法论，由于有了架构完整、逻辑严密的学习方法论和知识体系，使得在此基础上能够快速生成高质量产出文章（针对 `LLM-Wiki` #3）。

- 在 **KOS** 方面，采用了立体化目录层次结构（针对 `LLM-Wiki` #1），还融合了**剪藏（Clippings）**、**随手记（Inbox）**、**日记（Diary）**（针对 `LLM-Wiki` #2），同时加入了本人开发的多个 Obsidian 插件、脚本和模板，比如**间隔复习提醒（SR Reminder）**、**间隔复习评估脚本（sr-evaluate.js）**、**TASK 自动化脚本提醒（Shell Reminder）**（针对 `LLM-Wiki` #4），最后是 **AI 会话记录脚本（conversation-logger.js）**（针对 `LLM-Wiki` #5）。
...
- **AI 会话记录脚本（conversation-logger.js）**：可完整记录与 AI 的会话，你还可以快速归类整理，方便日后搜索和回顾。
...
| `_scripts/conversation-logger.js` | AI 会话记录脚本，自动将对话完整存入 `logs/`    |
...
**事实核查（Fact-Check）**：AI 对 `sources/` 中的关键断言逐条标注可信度，标记需要二次确认的内容（`[?]`），并在 `AGENTS.md` 中记录待核实任务，避免错误信息悄无声息地扩散至 `concepts/` 和 `synthesis/`，最后由人亲自核查确认。
...
### 一、剪藏（Clippings）—— 先落地，后整理，建提醒

剪藏是知识流入系统的第一道门，核心原则是**零阻力捕获**，不在录入时纠结去处。

- **YouTube 视频**：通过 Chrome 扩展 `OB Web Clipper` 一键剪藏，把书签栏的网页内容自动落入 `INBOX/Clippings/<YT视频标题>`。

**建立整理提醒：**

- 剪藏后，立即新建一个 `reminder` 类型的 Task，设定当晚处理时间。
- 若当日剪藏量较大，可在日记中新建**记忆卡片**，统一标注"今晚整理剪藏"，通过 SR Reminder 到点弹出提醒。

> 技巧：提醒时间如果没有日期、只有时间，表示每天这个时间都会提醒。

### 二、定位挂载

提醒触发后，我正式开始整理。此步骤的核心是：**把新知识挂载到层次清晰的知识树上，避免形成知识孤岛。**

1. 在知识树中找到所属领域节点（如无，新建对应文件夹）；
2. 在该节点下新建 **FAST 分子笔记本**（文件夹名称为领域名、slug 格式）；
3. 运行 `/llm-wiki-init`，由 LLM 初始化并生成 Wiki 架构；
4. 将剪藏文件移动至 `raw/`，清空原 `INBOX/Clippings/` 中对应条目。

### 三、摄入（Ingest）—— 原子化与分析

运行 `ingest`，AI 全文阅读原始资料，自动生成：
...
- ⚠️ 确认图片、附件等资源是否已正确加载至 `asset/`，避免后续链接失效。

### 四、健康检查（Lint）

`Ingest` 完成后，笔记本尚未进入"可信状态"。运行 `lint`，执行三层校验：

- **事实核查**：对 `sources/` 中的关键断言标注可信度，标记存疑内容（`[?]`）；
- **矛盾清除**：新旧节点冲突时，并列存档两种说法，待 Synthesis 阶段裁决；
- **结构检查**：扫描孤立节点、空占位文件、失效链接、重复概念，输出 `lint-report`。

处理所有报告问题后，笔记本才算摄入完毕。

### 五、生成 FAST 分子笔记

基于已通过 Lint 校验的 `concepts/` 和 `entities/`，运行 `FAST 6+1` 维度扩展，完成**分子化与综合**过程，生成结构完整、逻辑自洽的分子笔记。

### 六、思考、综合与产出（最后新建记忆卡片）

这是 LLM-FAST 区别于传统 LLM-Wiki 的核心价值所在。

在分子笔记本积累到一定密度后，开始主动**思考与综合**：运用 `wiki/synthesis/` 和 `wiki/outputs/` 产出新笔记——深度分析、对比洞察、教程草稿……逐步将"摄入的信息"升华为"可输出的知识"，持续丰富该 FAST 分子笔记本。

- 对该 FAST 或其中重要页面，在日记中**新建记忆卡片**，纳入间隔复习（SR）流程，防止新知识在摄入后就被遗忘。

### 七、每周复盘：半随机游走 + AI 洞察

每周定期触发复盘工作流：

- **半随机游走**：系统优先推荐"重要但久未回顾"的知识节点，激活沉睡记忆；
- **AI 洞察**：借助 LLM 发现知识库中的盲区、潜在关联与新学习方向；
- **开启新一轮循环**：由洞察驱动新一轮剪藏（捕获）与思考（产出）的闭环。
