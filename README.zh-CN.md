...
浏览器书签栏（因为很明显！所以不必提醒。清空欲望所带来的紧迫感、促使你非要清空改书签栏不可、否则就不舒服）

Youtube 视频（Chrome扩展： OB Web Clipper）=》OB：INBOX/Clippings/<YT视频标题>

新建Task：提醒晚上整理该剪藏
如果很多剪藏：在日记新建一个记忆卡片、统一提醒晚上整理该剪藏

### 定位挂载

提醒到点后，开始整理剪藏：在知识树适当分支（如无则新建文件夹），新建FAST文件夹（名称为该领域、slug格式）
然后llm初始化、生成wiki架构
最后把剪藏文件移动到raw，原来的剪藏文件夹清空

### 运行ingest、生成概念、实体等（在日记为该FAST或其中某些重要页面新建记忆卡片）
...
### 思考和综合、生成产出新笔记、逐渐丰富该FAST分子笔记本的过程！！！


### 每周复盘：随机游走； AI洞察找出新方向、由我开始新一轮学习（剪藏）和思考（产出）过程
...
**先落地，后整理。** 剪藏是知识流入系统的第一道门，核心原则是**零阻力捕获**，不在录入时纠结去处。

**两种主要来源：**

- **浏览器书签栏**：书签栏天然具有"看着碍眼、不清空不舒服"的心理压力，这种紧迫感是驱动整理的动力——善用它。书签不需要额外提醒，视觉压迫本身就是提醒。
- **YouTube 视频**：通过 Chrome 扩展 OB Web Clipper 一键剪藏，自动落入 `INBOX/Clippings/<YT视频标题>`。
